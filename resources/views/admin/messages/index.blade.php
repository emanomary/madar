                                            ...
                                            @if($userMessages->count() > 0)
                                                ...
                                                @foreach($userMessages as $index => $userMessage)
                                                    <tr>
                                                        <td>{{$index + 1}}</td>
                                                        <td>
                                                            <a href="{{route('admin.messages.show',$userMessage->id)}}">
                                                                {{$userMessage->name}}
                                                            </a>
                                                        </td>
                                                        <td>{{$userMessage->mobile}}</td>
...
@section('script')
    ...
    <script>
        $('#deleteModal').on('show.bs.modal', function (event) {
            ...
            var message_id = button.data('messageid')

            var modal = $(this)
            // modal.find('.modal-footer #user_id').val(user_id)
            modal.find('form').attr('action','/messages/destroy/' + message_id);
        })
    </script>
@endsection
